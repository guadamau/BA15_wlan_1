/* *****************************************************************************
   ...
   Purpose:    Specifiy the values that have to be extracted 
               ...
               These parameters are initialized to run a unit test.
...
void init_proc_properties( void )
{
   ...
   * configuration files.
   * */
  strncpy( init_paths[ INDEX_UTIME        ], "t_stat",          PATH_LEN );
  strncpy( init_paths[ INDEX_STIME        ], "t_stat",          PATH_LEN );
  strncpy( init_paths[ INDEX_IF_NAME_IF0  ], "t_dev",           PATH_LEN );
  strncpy( init_paths[ INDEX_RX_BYTES_IF0 ], "t_dev",           PATH_LEN );
  strncpy( init_paths[ INDEX_TX_BYTES_IF0 ], "t_dev",           PATH_LEN );


  strncpy( init_search[ INDEX_UTIME        ], "",               PATH_LEN );
  strncpy( init_search[ INDEX_STIME        ], "",               PATH_LEN );
